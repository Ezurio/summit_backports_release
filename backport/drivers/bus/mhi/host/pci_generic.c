...
static void mhi_pci_reset_prepare_notify(struct pci_dev *dev, bool prepare){
	if (prepare)
		mhi_pci_reset_prepare(dev);
	else
		mhi_pci_reset_done(dev);
}
#endif
...
struct pci_error_handlers mhi_pci_err_handler = {
	...
	.reset_notify = mhi_pci_reset_prepare_notify,
#endif
