...
static void woal_pcie_reset_prepare_notify(struct pci_dev *dev, bool prepare){
	if (prepare)
		woal_pcie_reset_prepare(dev);
	else
		woal_pcie_reset_done(dev);
}
#endif
...
struct pci_error_handlers woal_pcie_err_handler = {
		...
		.reset_notify = woal_pcie_reset_prepare_notify,
#endif
