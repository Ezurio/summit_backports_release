...
//prod-29036
static uint httxcfg_2 = 0;

//prod-29240
static uint disable_phy = 0;
					...
					moal_handle *handle)
{
	...
	while ((int)parse_cfg_get_line(data, size, line) != -1) {
		...
		//prod-29036
		} else if (strncmp(line, "httxcfg_2",
				   strlen("httxcfg_2")) == 0) {
			if (parse_line_read_int(line, &out_data) !=
			    MLAN_STATUS_SUCCESS)
				goto err;
			params->httxcfg_2 = out_data;
			PRINTM(MMSG, "httxcfg_2=%d\n", params->httxcfg_2);
		//prod-29240
		} else if (strncmp(line, "disable_phy",
			    strlen("disable_phy")) == 0 ) {
			if (parse_line_read_int(line, &out_data) !=
			    MLAN_STATUS_SUCCESS)
				goto err;
			params->disable_phy = out_data;
			PRINTM(MMSG, "disable_phy=%d\n", params->disable_phy);
		}
	}
...
static void woal_setup_module_param(moal_handle *handle, moal_mod_para *params)
{
	...
	//prod-29036
	handle->params.httxcfg_2 = httxcfg_2;
	if (params)
		handle->params.httxcfg_2 = params->httxcfg_2;
	//prod-29240
	handle->params.disable_phy = disable_phy;
	if (params)
		handle->params.disable_phy = params->disable_phy;
}
...
void woal_init_from_dev_tree(void)
{
	...
	for_each_property_of_node (dt_node, prop) {
		...
		//prod-29036
		} else if (!strncmp(prop->name, "httxcfg_2",
				   strlen("httxcfg_2"))) {
			if (!of_property_read_u32(dt_node, prop->name, &data)) {
				PRINTM(MMSG, "httxcfg_2=%d\n", data);
				httxcfg_2 = data;
		...
		//prod-29240
		} else if (!strncmp(prop->name, "disable_phy",
				   strlen("disable_phy"))) {
			if (!of_property_read_u32(dt_node, prop->name, &data)) {
				PRINTM(MMSG, "disable_phy=%d\n", data);
				disable_phy = data;
...
//prod-29036
module_param(httxcfg_2, uint, 0);
MODULE_PARM_DESC(
	httxcfg_2,
	"<drv_mode:16><txcfg:16> Configure various TX 2.4 11n configuration settings for specified interfaces");
