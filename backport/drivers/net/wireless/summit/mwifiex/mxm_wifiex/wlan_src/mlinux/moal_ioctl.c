				     ...
				     mlan_fw_info *fw_info)
{
	...
	if (status == MLAN_STATUS_SUCCESS) {
		...
		if (priv->phandle->params.disable_phy & VNDR_DISABLE_PHY_2G) {
			info->param.fw_info.fw_bands &= ~(BAND_B | BAND_G | BAND_GN | BAND_GAC | BAND_GAX);
		}
		if (priv->phandle->params.disable_phy & VNDR_DISABLE_PHY_5G) {
			info->param.fw_info.fw_bands &= ~(BAND_A | BAND_AN | BAND_AAC | BAND_AAX);
		}
...
mlan_status woal_set_band(moal_private *priv, char *pband)
{
	...
	if (*pband == '0') {
		...
		if (!(radio_cfg->param.band_cfg.fw_bands & BAND_A) || 
			 //prod-29240
			 (priv->phandle->params.disable_phy & VNDR_DISABLE_PHY_5G)) {
		...
		if (priv->phandle->params.disable_phy & VNDR_DISABLE_PHY_2G) {
			PRINTM(MERROR, "Don't support 2G band\n");
			ret = MLAN_STATUS_FAILURE;
			goto done;
		}
		band = BAND_B | BAND_G;
