...
#ifdef CPTCFG_KERNEL_VERSION_CODE
#define CFG80211_VERSION_CODE CPTCFG_KERNEL_VERSION_CODE
#else
...
#define WIFI_STATUS_EMERGENCY_TEMP_REACHED 12
...
struct _moal_handle {
	...
	struct delayed_work emergency_reset_work;
#endif
	/** main state */
	t_u8 main_state;
	/** driver status */
...
#ifdef ANDROID_KERNEL
static inline void ezio_wakeup_source_init(struct wakeup_source *ws,
				      const char *name)
{
	ENTER();
...
static inline void ezio_wakeup_source_trash(struct wakeup_source *ws)
{
	ENTER();
