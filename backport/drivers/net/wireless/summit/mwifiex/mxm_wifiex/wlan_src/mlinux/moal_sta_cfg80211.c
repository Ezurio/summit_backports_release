			...
			t_u8 wait_option)
{
	...
	if (chan->band == IEEE80211_BAND_2GHZ) {
		...
		if ((priv->phandle->params.disable_phy & VNDR_DISABLE_PHY_2G)) {
			PRINTM(MERROR, "2G band unsupported.\n");
			ret = -EFAULT;
			goto done;
		}
		radio_cfg->param.band_cfg.config_bands = BAND_B | BAND_G | BAND_GN;
	}
	else {
		if ((priv->phandle->params.disable_phy & VNDR_DISABLE_PHY_5G)) {
			PRINTM(MERROR, "5G band unsupported.\n");
			ret = -EFAULT;
			goto done;
		}
		radio_cfg->param.band_cfg.config_bands = BAND_AN | BAND_A;
	}
...
void woal_regulatory_work_queue(struct work_struct *work)
{
	...
	if (priv && wiphy) {
		...
		band = priv->phandle->band;
		if (priv->phandle->fw_bands & BAND_B) {
			//prod-29240
					    ...
					    t_u8 wait_option)
{
	...
	if (!fw_info->force_reg && countryOTP[0] && countryOTP[1] &&
		...
		/* Check radio type and adjust rgpower file if necessary */
		/* Prod-27767 */
		if (MLAN_STATUS_SUCCESS == woal_vndr_get_radio_form(priv->phandle, &form) &&
			RID_ATTR_FRM_1218 == form ) {
			moal_handle *handle = priv->phandle;
			int len = strlen(PREFIX_1218);

			PRINTM(MMSG, "Radio is a %s\n", PREFIX_1218);
			if (handle->params.hw_name) {
				kfree(handle->params.hw_name);
				handle->params.hw_name = NULL;
			}

			handle->params.hw_name = kzalloc(len + 1, GFP_KERNEL);
			if (handle->params.hw_name == NULL) {
				PRINTM(MERROR, "Failed to alloc mem for hw_name: %s\n", PREFIX_1218);
				return MLAN_STATUS_FAILURE;				
			}
			moal_memcpy_ext(NULL, handle->params.hw_name, PREFIX_1218, len, len);
		}
...
mlan_status woal_register_cfg80211(moal_private *priv)
{
	...
	/* ezio remove radar_detect_widths */
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(3, 12, 0)
	if (IS_CARD9177(priv->phandle->card_type))
		cfg80211_iface_comb_ap_sta.radar_detect_widths = 0;
#endif
