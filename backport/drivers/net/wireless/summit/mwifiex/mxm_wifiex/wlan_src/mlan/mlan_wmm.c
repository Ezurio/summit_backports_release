...
static t_u32 wlan_wmm_get_byte_budget(pmlan_adapter pmadapter, t_u32 time_budget_us, t_u32 phy_rate_kbps)
{
	...
	//ezio added moal_do_div
	byte_budget = cbs->moal_do_div(((t_u64)phy_rate_kbps * time_budget_us), (8 * 1000u));
...
wlan_wmm_allocate_sta_table(pmlan_adapter pmadapter, t_u8 *ra)
{
	...
	sta_table->budget.byte_budget_init = wlan_wmm_get_byte_budget(pmadapter,
	...
	sta_table->budget.mpdu_with_amsdu_budget_init = 
		cbs->moal_do_div(((t_u64)sta_table->budget.mpdu_with_amsdu_pps_cap *
		sta_table->budget.time_budget_init_us), 1000000);

	//ezio added moal_do_div()
	sta_table->budget.mpdu_no_amsdu_budget_init = 
		cbs->moal_do_div(((t_u64)sta_table->budget.mpdu_no_amsdu_pps_cap *
			sta_table->budget.time_budget_init_us), 1000000);
						   ...
						   pmlan_private priv)
{
	...
	while ((list_entry = util_peek_list_nl(
		...
		const t_u32 max_pkts_by_airtime =
			...
			MV_ETH_FRAME_LEN;
		//ezio added moal_do_div()
		t_u32 sta_share = cbs->moal_do_div(
					  ...
					  HostCmd_TX_RATE_QUERY *rate)
{
	...
	if (phy_rate > 0) {
		...
		sta->budget.byte_budget_init = wlan_wmm_get_byte_budget(pmadapter,
