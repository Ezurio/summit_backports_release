...
#define LRD_SDIO_DESC        "Summit 60 Series Wireless SDIO Network Driver"
...
static int mwl_sdio_reset(struct mwl_sdio_card *card)
{
	struct sdio_func *func = card->func;
	int rc;
	...
	if (!mwl_gpio_set(card->reset_pwd, 0)) {
		msleep(SDIO_DEFAULT_POWERDOWN_DELAY_MS);
		mwl_gpio_set(card->reset_pwd, 1);
	}

	rc = mwl_sdio_mmc_hw_reset(func);
...
static int mwl_sdio_init(struct mwl_priv *priv)
{
	...
	if (!priv->pcmd_buf) {
		...
		if (!priv->pcmd_buf) {
			wiphy_err(priv->hw->wiphy, MWL_DRV_NAME
...
static int mwl_sdio_wakeup_card(struct mwl_priv *priv)
{
	int status;
	struct mwl_sdio_card *card = priv->intf;
	u8 cr;
...
static void mwl_sdio_wakeup_complete(struct work_struct *work)
{
...
static void mwl_sdio_enter_ps_sleep(struct work_struct *work)
{
	struct mwl_sdio_card *card = container_of(work,
...
mwl_process_txdesc(struct mwl_priv *priv, struct sk_buff *skb, struct mwl_dma_data **dma_data)
{
	...
	misalign = ((unsigned long)skb->data - (sizeof(struct mwl_tx_desc) + INTF_HEADER_LEN)) % MWL_DMA_ALIGN_SZ;
	align_pad =  (MWL_DMA_ALIGN_SZ - misalign) % MWL_DMA_ALIGN_SZ;
...
static int mwl_sdio_up_pwr(struct mwl_priv *priv)
{
	...
	rc = mwl_gpio_set(card->reset_pwd, 1);
	if (rc)
		return rc;
...
static int mwl_sdio_dbg_info (struct mwl_priv *priv, char *p, int size, int len)
{
	...
	if (mwl_gpio_valid(card->reset_pwd))
		len += scnprintf(p + len, size - len, "PMU_EN GPIO: %d\n", mwl_gpio_num(card->reset_pwd));
	...
	const struct sdio_device_id *id)
{
	...
	struct mwl_sdio_card *card;
	struct device_node *np, *of_node = dev_of_node(&func->dev);
	int rc;
	...
	if (of_node) {
		if (of_match_node(mwl_sdio_of_match_table, of_node))
	...
	card = devm_kzalloc(&func->dev, sizeof(struct mwl_sdio_card), GFP_KERNEL);
	if (!card) {
	...
	rc = mwl_gpio_init(&func->dev, of_node, &card->reset_pwd);
	if (rc) {
		if (rc != -EPROBE_DEFER)
			dev_err(&func->dev, "PMU_EN GPIO failed: %d\n", rc);
...
static int mwl_sdio_pm_worker(struct device *dev, int action)
{
	...
	if (action == MWL_PM_RESTORE_EARLY)
		mwl_gpio_recover(card->reset_pwd);
...
static int __init mwl_sdio_driver_init(void)
{
	...
	rc = mwl_gpio_init_legacy();
	if (rc)
		return rc;

	rc = sdio_register_driver(&mwl_sdio_driver);
	if (rc)
		mwl_gpio_release_legacy();
