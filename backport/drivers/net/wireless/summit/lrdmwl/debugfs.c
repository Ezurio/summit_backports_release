                                         ...
                                         size_t count, loff_t *ppos)
{
	struct mwl_priv *priv = (struct mwl_priv *)file->private_data;
	char dump = 0;
	char type = HOSTCMD_ACT_GET_MAX_TX_PWR;
	...
	if (count >= 3) {
		/* allow <arg><space><arg> or <arg><arg>*/
		if (copy_from_user(&type, ubuf + (count -2), 1)) {
			return -EFAULT;	
		}

		switch (type) {
			case '0':
				type = HOSTCMD_ACT_GET_TARGET_TX_PWR;
			break;
			case '1':
				type = HOSTCMD_ACT_GET_MAX_TX_PWR;
			break;
			default:
				pr_err("Invalid argument %x : 0 or 1 required\n",type);
				return -EINVAL;
			break;
		}
	}
	...
	{
		case '1':
			lrd_dump_max_pwr_table(priv, type);
		break;
		default :
			pr_err("Invalid argument : 1 needed\n");
			return -EINVAL;
