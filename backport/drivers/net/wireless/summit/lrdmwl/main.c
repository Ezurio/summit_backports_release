...
#define REGULATORY_DB_NAME  "regulatory_60.db"
...
{
	//Check region and CC in case either has changed.
	if (priv->reg.cc.region == cc->region && 
	    0 == memcmp(priv->reg.cc.alpha2, cc->alpha2, sizeof(priv->reg.cc.alpha2))) {
		//Aleady using, don't send again
		return;
	}

	wiphy_debug(priv->hw->wiphy,"Sending regulatory hint for %c%c (0x%x)\n", cc->alpha2[0], cc->alpha2[1], cc->region);

	//Fixup the channel set based on country/region
	lrd_fixup_channels(priv, cc);

	priv->reg.hint.region = cc->region;
	memcpy(&priv->reg.hint.alpha2, cc->alpha2, sizeof(priv->reg.hint.alpha2));
...
void lrd_dump_max_pwr_table(struct mwl_priv *priv, u8 type)
{
	...
	pr_info("Power Table - %s per RateId:\n", type == HOSTCMD_ACT_GET_TARGET_TX_PWR?"Target":"Max");
	...
	for (ch_index = 0; ch_index < ARRAY_SIZE(mwl_channels_24); ch_index++) {
		...
		rc = mwl_fwcmd_get_tx_powers(priv, &powlist[0], type,
...
static void ds_routine(struct timer_list *t)
{
...
void lrd_cc_event(struct work_struct *work)
{
	...
	wiphy_debug(hw->wiphy, "Checking CRDA for Country Code %c%c (0x%x).\n",
		info.alpha2[0], info.alpha2[1], info.region);
...
exit:
	if (valid) {
		wiphy_debug(hw->wiphy, "Country Code %c%c (0x%x) is valid.\n",
			info.alpha2[0], info.alpha2[1], info.region);
...
static void lrd_awm_routine(struct timer_list *t)
{
...
static int mwl_wl_init(struct mwl_priv *priv)
{
	...
	if (disable_phy & DISABLE_2G) {
		...
		priv->curr_channel = 36;
	}
	else if (disable_phy & DISABLE_5G) {
	...
	if (rc) {
		...
		if (priv->hw_data.fw_release_num == NOT_LRD_HW) {
			wiphy_err(hw->wiphy,
			     "Detected non Summit hardware: 0x%x\n", priv->hw_data.fw_release_num);
			rc = -ENODEV;
			goto err_wl_init;
		}
	}
	...
	rc = regulatory_load_regdb(REGULATORY_DB_NAME);
	if (rc) {
		wiphy_err(hw->wiphy, "Could not load regulatory database %s, aborting!\n",
			REGULATORY_DB_NAME);
	...
	struct device_node *of_node)
{
	struct ieee80211_hw *hw;
	struct mwl_priv *priv;
	const char *fwtype = fw_type;
	int rc = 0;

#ifdef CPTCFG_LRDMWL_FIPS
	if (fips_enabled && fips_wifi_enabled) {
		host_crypto_mode = 1;
	}
#endif

	if (host_crypto_mode) {
		mwl_mac80211_ops.set_key = NULL;
	}
	...
	if (of_node) {
		...
		of_property_read_u32(of_node, "disable-phy", &disable_phy);
		if ( disable_phy & DISABLE_2G) {
			priv->disable_2g = true;
			priv->curr_channel = 36;
		}
	...
	of_property_read_string(of_node, "fwtype", &fwtype);
...
void lrd_send_restart_event(struct wiphy *wiphy, struct ieee80211_vif *vif, u32 reason)
{
	struct sk_buff *skb;
...
module_param(fw_type, charp, 0);
MODULE_PARM_DESC(fw_type, "Firmware type <wifi-bt>: usb-uart, usb-usb, sdio-uart, sdio-sdio, pcie-uart, pcie-usb");
