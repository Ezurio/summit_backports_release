...
static int brcmf_sdio_bus_idle_time = BRCMF_DEFAULT_SDIO_IDLE_CONFIG;
module_param_named(bus_idle_time, brcmf_sdio_bus_idle_time, int, 0644);
...
#if LINUX_VERSION_IS_LESS(6,2,15) && !LINUX_VERSION_IN_RANGE(6,1,30, 6,2,0)
static int bp_brcmf_pd_brcmf_common_pd_remove(struct platform_device *pdev){
...
#if LINUX_VERSION_IS_LESS(6,2,15) && !LINUX_VERSION_IN_RANGE(6,1,30, 6,2,0)
	.remove = bp_brcmf_pd_brcmf_common_pd_remove,
#else
