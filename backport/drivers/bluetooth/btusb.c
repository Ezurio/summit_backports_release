...
static bool enable_autosuspend = IS_ENABLED(CPTCFG_BT_HCIBTUSB_AUTOSUSPEND);
static bool enable_poll_sync = IS_ENABLED(CPTCFG_BT_HCIBTUSB_POLL_SYNC);
...
#ifdef CPTCFG_BT_HCIBTUSB_INTEL
		       ...
		       const struct usb_device_id *id)
{
	...
	if (IS_ENABLED(CPTCFG_BT_HCIBTUSB_BCM) &&
...
#ifdef CPTCFG_BT_HCIBTUSB_MTK
	if (IS_ENABLED(CPTCFG_BT_HCIBTUSB_MTK) &&
	...
	if (IS_ENABLED(CPTCFG_BT_HCIBTUSB_RTL) &&
...
#ifdef CONFIG_DEV_COREDUMP
#if LINUX_VERSION_IS_LESS(6,8,0) && !(LINUX_VERSION_IS_EQU(5,14,0) && RHEL_RELEASE_IS_GEQ(9,5))
	.drvwrap = {
#endif
		.driver = {
#if LINUX_VERSION_IS_GEQ(4,17,0)
