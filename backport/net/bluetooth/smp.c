...
#if LINUX_VERSION_IS_GEQ(6,1,11)
	iov_iter_kvec(&msg.msg_iter, ITER_SOURCE, iv, 2, 1 + len);
#elif LINUX_VERSION_IS_GEQ(4,20,0)
...
static struct smp_chan *smp_chan_create(struct l2cap_conn *conn)
{
	...
	if (IS_ERR(smp->tfm_aes)) {
		bt_dev_err(hcon->hdev, "Unable to create AES crypto context");
		goto zfree_smp;
	}
...
static struct l2cap_chan *smp_add_cid(struct hci_dev *hdev, u16 cid)
{
	...
	if (IS_ERR(tfm_aes)) {
		bt_dev_err(hdev, "Unable to create AES crypto context");
		kfree_sensitive(smp);
