...
#elif LINUX_VERSION_IS_GEQ(4,11,0)
			   int flags, bool kern)
#else
			   int flags)
#endif
...
static int backport_iso_sock_sendmsg(struct kiocb *iocb, struct socket *sock,
				     struct msghdr *msg, size_t len){
	return iso_sock_sendmsg(sock, msg, len);
}
...
#if LINUX_VERSION_IS_LESS(4,1,0)
static int backport_iso_sock_recvmsg(struct kiocb *iocb, struct socket *sock,
				     struct msghdr *msg, size_t len,
				     int flags){
	return iso_sock_recvmsg(sock, msg, len, flags);
}
#endif /* LINUX_VERSION_IS_LESS(4,1,0) */
...
#else
	.sendmsg = backport_iso_sock_sendmsg,
#endif /* LINUX_VERSION_IS_GEQ(4,1,0) */
	
#if LINUX_VERSION_IS_GEQ(4,1,0)
	.recvmsg	= iso_sock_recvmsg,
#else
	.recvmsg = backport_iso_sock_recvmsg,
#endif /* LINUX_VERSION_IS_GEQ(4,1,0) */
