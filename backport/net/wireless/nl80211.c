...
#else
	[0] = { .u.strict_start_type = NL80211_ATTR_HE_OBSS_PD },
#endif
...
#else
 	[NL80211_ATTR_EHT_CAPABILITY] =
		{ .type = NLA_BINARY, .len = NL80211_EHT_MAX_CAPABILITY_LEN },
#endif
	...
	if (info->attrs[NL80211_ATTR_MLO_LINKS]) {
		...
		nla_for_each_nested(link,
			...
			if (!attrs[NL80211_ATTR_MLO_LINK_ID]) {
				err = -EINVAL;
				NL_SET_BAD_ATTR(genl_info_extack(info), link);
				goto free;
			}
...
static int nl80211_pre_doit(__genl_const struct genl_split_ops *ops,
			    struct sk_buff *skb,
			    struct genl_info *info)
{
	struct cfg80211_registered_device *rdev = NULL;
...
static void nl80211_post_doit(__genl_const struct genl_split_ops *ops,
			      struct sk_buff *skb,
			      struct genl_info *info)
{
...
#endif
#if LINUX_VERSION_IS_GEQ(6,1,0) || (LINUX_VERSION_IS_EQU(5,14,0) && RHEL_RELEASE_IS_GEQ(9,3))
